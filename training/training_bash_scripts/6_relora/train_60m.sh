...
CUDA_VISIBLE_DEVICES=4 torchrun --standalone --nproc_per_node 1 training/torchrun_main_common.py \
    ...
    --run_name "relora_60m" \
    --save_dir checkpoints/relora-60m
...
CUDA_VISIBLE_DEVICES=4 torchrun --standalone --nproc_per_node 1 training/torchrun_main_common.py \
    --method relora \
    --model_config training/configs/llama_configs_relora/llama_60m.json \
    --lr 0.001 \
    --lora_r 128 \
    --relora 1000 \
    --reset_optimizer_on_relora False \
    ...
    --method relora \
    --scheduler cosine_restarts \
    --run_name "relora_60m_padtoken-1" \
    --save_dir checkpoints/relora-60mpadtoken-1