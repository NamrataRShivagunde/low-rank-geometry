#!/bin/bash
# PCA k=5,10,100 for 130m only. ~75 min on one GPU.
# Usage: bash loss-landscape/scripts/bash_scripts/pca_k5_k10_k100_130m.sh <GPU_ID>
set -euo pipefail
GPU_ID=${1:-2}
...
for K in 5 10 100; do
    ...
    for m in ${METHODS}; do
        for c in ${CKPTS}; do
            run_single_k ${K} ${m} "CHECKPOINTS/models-130m/${m}-130m/${c}" "results/pca-verification-k${K}/130m/${m}/${c}"
        done
    done
done
echo "130m done!"
