#!/bin/bash
# PCA top-20 directions for 130m WITHOUT sigma weighting (svd_weight_mode=none).
# Results go to results/pca-verification-k20-nosigma/130m/
#
# Usage: bash loss-landscape/scripts/bash_scripts/pca_top20_130m_nosigma.sh <GPU_ID> [method1 method2 ...]
set -euo pipefail
GPU_ID=${1:-3}
shift || true
...
CKPTS="model_2000 model_4000 model_6000 model_8000 model_10000 model_12000 model_14000 model_16000 model_18000 model_20000"
RESULT_ROOT="results/pca-verification-k20-nosigma"

for m in ${METHODS}; do
    for c in ${CKPTS}; do
        out="${RESULT_ROOT}/130m/${m}/${c}"
        if [ -f "${out}/stats.json" ]; then
            ...
            CUDA_VISIBLE_DEVICES=${GPU_ID} conda run -n =ll-training --no-capture-output python ${SCRIPT} \
                --model ${m} --checkpoint "CHECKPOINTS/models-130m/${m}-130m/${c}" --tokenizer t5-base \
...
echo "$(date '+%H:%M:%S') 130m nosigma done!"
