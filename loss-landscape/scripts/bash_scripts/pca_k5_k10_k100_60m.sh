#!/bin/bash
# PCA k=5,10,100 for 60m only. ~40 min on one GPU.
# Usage: bash loss-landscape/scripts/bash_scripts/pca_k5_k10_k100_60m.sh <GPU_ID>
set -euo pipefail
GPU_ID=${1:-1}
...
for K in 5 10 100; do
    ...
    for m in ${METHODS}; do
        for c in ${CKPTS}; do
            run_single_k ${K} ${m} "CHECKPOINTS/models-60m/${m}-60m/${c}" "results/pca-verification-k${K}/60m/${m}/${c}"
        done
    done
done
echo "60m done!"
