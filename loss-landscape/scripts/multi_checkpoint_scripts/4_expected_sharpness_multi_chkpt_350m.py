...
results/loss-landscape-2d-PCA/models-350m/ (or a custom root).
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Batch compute expected sharpness for 350m checkpoints")
    parser.add_argument(
        "--landscape_root",
        type=Path,
        default=Path("results/loss-landscape-2d-PCA/models-350m"),
    ...
    for idx, (model_type, checkpoint_name, checkpoint_path) in enumerate(checkpoints, start=1):
        print(f"\nProcessing checkpoint {idx}/{len(checkpoints)}: {model_type}/{checkpoint_name}")
    ...
    sharpness_summary_json = summary_dir / "expected_sharpness_350m_summary.json"
    sharpness_summary_csv = summary_dir / "expected_sharpness_350m_summary.csv"
    variance_summary_json = summary_dir / "average_variance_350m_summary.json"
    variance_summary_csv = summary_dir / "average_variance_350m_summary.csv"
