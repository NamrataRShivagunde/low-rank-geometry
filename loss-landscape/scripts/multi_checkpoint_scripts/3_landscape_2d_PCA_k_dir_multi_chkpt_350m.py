#!/usr/bin/env python3
"""
Batch script to run landscape analysis on all checkpoints in CHECKPOINTS/models-350m/

Structure:
    CHECKPOINTS/models-350m/
    ├── llama-60m/
    │   ├── model_1000/
    │   ├── model_2000/
    │   └── ...
...
Results are saved to:
    results/loss-landscape-2d/models-350m/
...
def run_landscape_for_checkpoint(
    ...
    cmd = [
        "python",
        "loss-landscape/scripts/single_checkpoint_scripts/3_landscape_2d_pca_topk_components.py",
        "--model", model_type,
        "--checkpoint", str(checkpoint_path),
        "--task", args.task,
        "--tokenizer", args.tokenizer,
        "--top_k_components", str(max(1, args.top_k_components)),
        ...
        "--band_std_mult", str(args.band_std_mult),
    ]
    ...
    if args.save_per_direction_plots:
        cmd.append("--save_per_component_plots")
...
def main():
    parser = argparse.ArgumentParser(
        description="Batch run PCA top-k/component-range landscape analysis on all checkpoints in CHECKPOINTS/models-350m/"
    )
    ...
    parser.add_argument(
        ...
        help="Root directory containing all checkpoint folders (default: CHECKPOINTS/models-350m)"
    )
    parser.add_argument(
        "--output_root",
        type=Path,
        default=Path("results/loss-landscape-2d-PCA-k"),
        help="Root output directory for results (default: results/loss-landscape-2d-PCA-k)"
    )
    parser.add_argument(
        "--task",
        type=str,
    ...
    parser.add_argument(
        "--mode",
        type=str,
        choices=["2D", "3D", "2D_PCA"],
        default="2D_PCA",
        help="Landscape plot mode (default: 2D_PCA)"
    )
    parser.add_argument(
        "--top_k_components",
        type=int,
        default=100,
        help="For 2D_PCA mode, average components 1..K (default: 100)"
    )
    parser.add_argument(
        "--component_start",
        type=int,
        default=None,
        help="Optional start component index (1-based, inclusive)"
    )
    parser.add_argument(
        "--component_end",
        type=int,
        default=None,
        help="Optional end component index (1-based, inclusive)"
    )
