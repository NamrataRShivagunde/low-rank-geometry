...
from .GCG import GCGAttack, get_embeddings, get_embedding_matrix, get_nonascii_toks
from .BaseAttack import BaseAttacker
from models import BaseModel

__all__ = [
    "ImprovedGCG",
    "change_dataset_to_i_gcg_target",
    "IGCGOnlyEnumerate",
    "WarmStartAttacker",
    "IGCGOnlyEnumerateForDiffusionLanguageModel",
]
...
class IGCGOnlyEnumerateForDiffusionLanguageModel(IGCGOnlyEnumerate):
    def __init__(self, *args, **kwargs):
        super(IGCGOnlyEnumerateForDiffusionLanguageModel, self).__init__(*args, **kwargs)
    ...
    def attack(self):
        ...
        for step in range(1, self.num_steps + 1):
            ...
            self.target_ids = input_ids[target_slice].clone()
            input_ids[target_slice] = 126336
    ...
    def token_gradients(self, input_ids, input_slice, target_slice, loss_slice):
        """
        和IGCGOnlyEnumerate.token_gradients唯一的区别：记录了target
        """
        model = self.models[0].model
        embed_weights = get_embedding_matrix(model)
        # 这里调用防御的forward
        ...
        # 和IGCGOnlyEnumerate.token_gradients唯一的区别
        # 这里不计算梯度, 直接把梯度全部设置成0.
        grad = torch.zeros(input_ids[input_slice].shape[0], embed_weights.shape[0], device=model.device)
    ...
    def target_loss(self, batch_input_ids, logits, target_slice, control_slice):
        ...
        target = self.target_ids.unsqueeze(0).expand(logits.shape[0], self.target_ids.numel())
        loss = crit(logits[:, loss_slice, :].transpose(1, 2), target)
