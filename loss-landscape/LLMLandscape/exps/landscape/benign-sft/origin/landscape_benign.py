...
parser.add_argument("--ckpt_step", type=str, default="650")
args = parser.parse_args()
# if args.model == "qwen":
#     model = Qwen2("Qwen/Qwen2.5-7B-Instruct")
#     worst_model = Qwen2(f"./results/benign-harmful-sft/benign/benign-5e-05-qwen/checkpoint-650/")
# elif args.model == "llama":
#     model = Llama31()
#     worst_model = Llama31(f"./results/benign-harmful-sft/benign/benign-5e-05-llama/checkpoint-850/")
# else:
#     model = Mistral()
#     worst_model = Mistral(f"./results/benign-harmful-sft/benign/benign-5e-05-mistral/checkpoint-350/")
if args.model == "qwen":
    # model = Qwen2("Qwen/Qwen2.5-7B")
    # worst_model = Qwen2(f"Qwen/Qwen2.5-7B-Instruct")
    model = Qwen2("Qwen/Qwen2.5-Math-7B")
    worst_model = Qwen2(f"Qwen/Qwen2.5-Math-7B-Instruct")
elif args.model == "llama":
    model = Llama31()
    worst_model = Llama31(f"./results/benign-harmful-sft/benign/benign-5e-05-llama/checkpoint-850/")
else:
...
for task in ["advbench", "humaneval", "mmlu"]:
    saving_name = task + "-" + args.model + f"-step-{args.ckpt_step}"
