...
"""
deepspeed --include localhost:1,2 landscape_worst_train.py --ddp_timeout 180000000
"""

dist.init_process_group(backend="nccl", timeout=datetime.timedelta(seconds=7200))

os.environ["WANDB_PROJECT"] = "worst-landscape"
...
from tester import lm_eval_humaneval, lm_eval_mmlu, lm_eval_gsm8k, test_vanilla_harmful_output_rate
...
class NegativeTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        loss = super(NegativeTrainer, self).compute_loss(model, inputs, return_outputs, num_items_in_batch)
        if isinstance(loss, tuple):
            return loss
        loss = -loss
        regularization = self.compute_distance(model, ori_model)
        trainer.log(
            {
                "ce_loss": loss.item(),
                "l2_reg": regularization.item(),
            }
        )
        return loss + regularization

    @staticmethod
    def compute_distance(a: nn.Module, b: nn.Module) -> Tensor:
        loss, count = 0, 0
        for p_a, p_b in zip(a.parameters(), b.parameters()):
            loss += NegativeTrainer.criterion(p_a, p_b)
            count += 1
        loss /= count
        return loss
...
model_name = "mistral"
assert model_name in ["useless", "llama", "mistral"]
...
ori_model.to(f"cuda:{local_rank}").requires_grad_(False)
ori_generate_function = model.generate


learning_rate = 5e-5
exp_name = f"worst-{learning_rate}-{model_name}"

training_args = TrainingArguments(
    output_dir=f"./results/benign-harmful-sft/worst/{exp_name}",
    eval_strategy="steps",
    logging_dir=f"./logs/benign-harmful-sft/worst/{exp_name}",
    eval_steps=5,
    save_steps=5,
    logging_steps=1,
